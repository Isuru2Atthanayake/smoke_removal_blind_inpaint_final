...
from datasets.smoke_dataset import train as sample_graffiti
...
class Tester:
    ...
    def __init__(self, cfg):
        ...
        self.transform = transforms.Compose([transforms.Resize(self.opt.DATASET.SIZE) if self.opt.DATASET.NAME.lower() == "SmokeDataset" else transforms.RandomCrop(self.opt.DATASET.SIZE, pad_if_needed=True, padding_mode="reflect"),
    ...
    def put_graffiti(self):
        ...
        masks = self.tensorize(resizer(Image.fromarray(sample.mask))).unsqueeze(0).cuda()
        graffiti_img = self.tensorize(resizer(Image.fromarray(sample.smoke))).unsqueeze(0).cuda()
